...
async function callSection(client, sectionName, userPrompt, ctx, schema, { maxTokens = 4096, model = MODEL } = {}) {
  ...
  try {
    ...
    console.log(`[reportAgent] "${sectionName}" completed in ${elapsed}s (${model})`)
    const text = response.content.filter(b => b.type === 'text').map(b => b.text).join('')
    return JSON.parse(text)
  } catch (err) {
    const elapsed = ((Date.now() - start) / 1000).toFixed(1)
    console.error(`[reportAgent] Section "${sectionName}" failed after ${elapsed}s:`, err.message)
    return null
  }
}
...
Requirements:
- "paragraphs": Exactly 4 paragraphs, 250-350 words total (STRICT MAXIMUM: 350 words — do not exceed)
...
Return JSON:
{
  ...
  return await callSection(client, 'scorecard_explanation', prompt, ctx, SCORECARD_EXPLANATION_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, `pillar_${pillarId}`, prompt, ctx, PILLAR_NARRATIVE_SCHEMA, { model: MODEL_FAST })
}
...
Return valid JSON matching the schema exactly.`
  ...
  return await callSection(client, 'competitive_positioning', prompt, ctx, COMPETITIVE_POSITIONING_SCHEMA, { maxTokens: 8192 })
}
...
Return JSON with all fields: narrative, why_factors, must_improve, scenarios_narrative, raps_narrative_bullets, scenario_analysis, section_takeaway, data_source ("raps_calculation").`
  ...
  return await callSection(client, 'raps_narrative', prompt, ctx, RAPS_NARRATIVE_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON with all fields: phase1_title, phase1_items, phase2_title, phase2_items, phase3_title, phase3_items, introduction, roadmap_rationale, qualitative_coverage_table, section_takeaway, data_source ("ai_synthesis").`
  ...
  return await callSection(client, 'action_roadmap', prompt, ctx, ACTION_ROADMAP_SCHEMA, { maxTokens: 16384, model: MODEL_FAST })
}
...
Return JSON with all fields: causal_links, reinforcing_loops, narrative, introduction, primary_causal_chain, reinforcing_loop, intervention_sequence, section_takeaway, data_source ("ai_synthesis").`
  ...
  return await callSection(client, 'cross_pillar_interaction', prompt, ctx, CROSS_PILLAR_SCHEMA, { maxTokens: 8192, model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'strategic_signals', prompt, ctx, STRATEGIC_SIGNALS_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'org_systems_readiness', prompt, ctx, ORG_SYSTEMS_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'advisory_workstream', prompt, ctx, ADVISORY_WORKSTREAM_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'closing_observation', prompt, ctx, CLOSING_OBSERVATION_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'diagnostic_interpretation', prompt, ctx, DIAGNOSTIC_INTERPRETATION_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON:
{
  ...
  return await callSection(client, 'snapshot_interpretations', prompt, ctx, SNAPSHOT_INTERPRETATIONS_SCHEMA, { model: MODEL_FAST })
}
...
Return JSON with all required fields and data_source "ai_synthesis".`
  ...
  return await callSection(client, 'expected_outcomes', prompt, ctx, EXPECTED_OUTCOMES_SCHEMA, { model: MODEL_FAST })
}
...
export async function generateDiagnosticReport(data) {
  ...
  console.log(`[reportAgent] Total report generation: ${totalElapsed}s`)
