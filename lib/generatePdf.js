...
import { PILLAR_WEIGHTS, PILLAR_NAMES, MATURITY_BANDS, DIAGNOSTIC_PILLARS } from '@/lib/constants'
...
export async function generateClientPdf({ scores, report, project, screenerResponses, diagnosticResponses, thresholds }) {
  ...
  function bulletPoint(text, opts = {}) {
    if (!text) return
    text = String(text)
    const { indent = 0, fontSize = 9, color = COLORS.dark } = opts
    doc.setFontSize(fontSize)
    doc.setTextColor(...color)
  ...
  function drawWrappedTable(headers, rows, opts = {}) {
    const { colWidths, fontSize = 8, headerFontSize = 8, headerColor, boldFirstCol = false } = opts
    const totalCols = headers.length
    const cws = colWidths || headers.map(() => contentW / totalCols)
    ...
    for (let ri = 0; ri < rows.length; ri++) {
      ...
      const cellLines = row.map((cell, ci) => {
        ...
        return doc.splitTextToSize(text, cws[ci] - 4)
      })
  ...
  const fmtUSD = (v) => v ? `US$${Number(v).toLocaleString()}` : null
  const remainingTarget = (sr.q18 && sr.q19) ? fmtUSD(Number(sr.q18) - Number(sr.q19)) : null
  ...
  for (const [pid, data] of sortedPillars) {
    ...
    // Band-colored progress bar (full width)
    doc.setFillColor(...color)
    ...
    if (rankings && Array.isArray(rankings) && rankings.length > 0 && pillarDef) {
      ...
      drawWrappedTable(qaHeaders, qaRows, { colWidths: qaColWidths, fontSize: 8, headerColor: COLORS.dark })
    }
    ...
    if (hasStrengthNarrative) {
      ...
      // Render as paragraph text (join all points)
      bodyText(pd.strength_points.join(' '), { fontSize: 9 })
    } else {
    ...
    // Find low-scoring questions (score 1 or 2) for structured friction point display
    const lowScoreQs = pillarDef ? pillarDef.questions
      .filter(q => q.type === 'scored')
      .map((qDef, qi) => {
    ...
    if (pd?.critical_friction_points?.length > 0 && lowScoreQs.length > 0) {
      // Structured format: question title, selected answer, interpretation
      for (let fi = 0; fi < lowScoreQs.length; fi++) {
        const lq = lowScoreQs[fi]
        ...
        doc.setFont('helvetica', 'normal')
        const interpLines = interpretation ? doc.splitTextToSize(interpretation, contentW - 12) : []
        const boxH = (qTitleLines.length * 4) + (ansLines.length * 3.5) + (interpLines.length * 3.5) + 8

        checkSpace(boxH + 4)

        // Box background + coral left border
      ...
      for (const cfp of pd.critical_friction_points) {
        checkSpace(18)
        const cfpLines = doc.splitTextToSize(cfp, contentW - 12)
  ...
  if (cpi?.primary_causal_chain?.nodes?.length >= 2) {
    ...
    for (let ni = 0; ni < pccNodeCount; ni++) {
      ...
      // Node box — white fill, coral border, rounded
      doc.setFillColor(255, 255, 255)
      ...
      if (ni < pccNodeCount - 1) {
        ...
        if (typeof node === 'object' && node.connector_label) {
          ...
          doc.setFont('helvetica', 'italic')
          doc.setTextColor(...COLORS.medium)
          // Truncate connector label to 2-3 words
          let connText = node.connector_label
          const connWords = connText.split(' ')
          if (connWords.length > 3) connText = connWords.slice(0, 3).join(' ')
          doc.text(connText, (ax1 + ax2) / 2, ay - 3, { align: 'center' })
        }
  ...
  if (cpi?.reinforcing_loop?.nodes?.length >= 3) {
    ...
    for (let ni = 0; ni < Math.min(rlNodeCount, 4); ni++) {
      ...
      // Node box — white fill, coral border
      doc.setFillColor(255, 255, 255)
      doc.setDrawColor(...COLORS.coral)
      doc.setLineWidth(0.8)
  ...
  function assessmentText(text) {
    ...
    doc.setFont('helvetica', 'normal')
    const lines = doc.splitTextToSize(text, contentW - prefixW)
    doc.text(lines[0], margin + prefixW, y)
    y += 4
    if (lines.length > 1) {
      const restLines = doc.splitTextToSize(text.substring(lines[0].length).trim(), contentW)
      for (const line of restLines) {
        checkSpace(4)
  ...
  if (cpData) {
    ...
    if (cpData.comparative_company_facts?.column_headers?.length && cpData.comparative_company_facts?.rows?.length) {
      ...
      const ccfColWidths = ccfHeaders.map(() => contentW / ccfHeaders.length)
      const ccfRows = ccf.rows.map(r => [r.field, ...(r.values || [])])
      drawWrappedTable(ccfHeaders, ccfRows, { colWidths: ccfColWidths, fontSize: 7.5, boldFirstCol: true })
    ...
    if (cpData.growth_evidence_signals?.column_headers?.length && cpData.growth_evidence_signals?.rows?.length) {
      ...
      const gesColWidths = gesHeaders.map(() => contentW / gesHeaders.length)
      const gesRows = ges.rows.map(r => [r.field, ...(r.values || [])])
      drawWrappedTable(gesHeaders, gesRows, { colWidths: gesColWidths, fontSize: 7.5, boldFirstCol: true })
    ...
    if (cpData.positioning_analysis) {
      ...
      bodyText(cpData.positioning_analysis, { fontSize: 9 })
    }
    ...
    if (cpData.recommended_positioning_direction) {
      ...
      // Guidance as body text
      if (rpd.guidance) {
        bodyText(rpd.guidance, { fontSize: 9 })
      }
      ...
      // Caveat as body text
      if (rpd.caveat) {
        bodyText(rpd.caveat, { fontSize: 9 })
      }
    }
  ...
  if (raps?.score !== undefined) {
    ...
    const kiRows = [
      ['Revenue Target', raps.revenueTarget ? `US$${Number(raps.revenueTarget).toLocaleString()}` : 'N/A'],
      ['YTD Invoiced', raps.revenueInvoiced ? `US$${Number(raps.revenueInvoiced).toLocaleString()}` : 'N/A'],
      ['Remaining', raps.revenueRemaining ? `US$${Number(raps.revenueRemaining).toLocaleString()}` : 'N/A'],
    ...
    for (let mi = 0; mi < countries.length; mi++) {
      ...
      function marketTable(rows) {
        ...
        for (const [label, value] of rows) {
          ...
          doc.rect(margin + labelW, y, valW, 8, 'S')
          doc.setFontSize(8.5)
          doc.setFont('helvetica', 'bold')
          doc.setTextColor(...COLORS.dark)
          doc.text(String(label), margin + 4, y + 5.5)
          doc.setFont('helvetica', 'normal')
          const valStr = String(value || '')
          const valTrunc = valStr.length > 60 ? valStr.substring(0, 60) + '..' : valStr
          doc.text(valTrunc, margin + labelW + 4, y + 5.5)
          y += 8
        }
        y += 4
      }
  ...
  // Organisation Readiness: P8 — with score on right
  {
    ...
    if (p8Part?.content) {
      bodyText(p8Part.content, { fontSize: 9 })
    } else if (report?.pillar_narratives?.p8) {
      const p8Narr = report.pillar_narratives.p8
      bodyText(typeof p8Narr === 'string' ? p8Narr : (p8Narr?.narrative || ''), { fontSize: 9 })
    }
  }
  ...
  // Systems Infrastructure: P9 — with score on right
  {
    ...
    if (p9Part?.content) {
      bodyText(p9Part.content, { fontSize: 9 })
    } else if (report?.pillar_narratives?.p9) {
      const p9Narr = report.pillar_narratives.p9
      bodyText(typeof p9Narr === 'string' ? p9Narr : (p9Narr?.narrative || ''), { fontSize: 9 })
    }
  }
  ...
  if (osrDetailed?.readiness_paradox) {
    ...
    y += 6
    bodyText(osrDetailed.readiness_paradox, { fontSize: 9 })
  }
  ...
  if (osrDetailed?.ai_readiness) {
    ...
    y += 6
    bodyText(osrDetailed.ai_readiness, { fontSize: 9 })
  }
  ...
  if (osrDetailed?.summary) {
    ...
    y += 6
    bodyText(osrDetailed.summary, { fontSize: 9 })
  }
  ...
  if (report?.strategic_signals) {
    ...
    if (ss?.narrative) {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(...COLORS.dark)
      const impPrefix = 'Implication: '
      const impPrefixW = doc.getTextWidth(impPrefix)
      doc.text(impPrefix, margin, y)
      doc.setFont('helvetica', 'normal')
      const impLines = doc.splitTextToSize(ss.narrative, contentW - impPrefixW)
      doc.text(impLines[0], margin + impPrefixW, y)
      y += 4
      if (impLines.length > 1) {
        const impRest = doc.splitTextToSize(ss.narrative.substring(impLines[0].length).trim(), contentW)
        for (const rl of impRest) {
          checkSpace(4)
  ...
  if (aw && aw.recommended_engagement && aw.recommended_engagement !== 'N/A') {
    ...
    if (aw.advisory_rationale) {
      ...
      bodyText(aw.advisory_rationale, { fontSize: 9 })
    }
  ...
  if (expectedOutcomes) {
    ...
    if (expectedOutcomes.revenue_confidence_intro) {
      bodyText(expectedOutcomes.revenue_confidence_intro, { fontSize: 9 })
    }
    ...
    if (expectedOutcomes.raps_context) {
      bodyText(expectedOutcomes.raps_context, { fontSize: 9 })
    }
