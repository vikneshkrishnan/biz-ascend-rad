...
const BAND_GRADIENTS = {
  strong:     { start: [29, 107, 114],  end: [42, 144, 153] },
  developing: { start: [74, 124, 89],   end: [107, 143, 113] },
  fragile:    { start: [139, 105, 20],  end: [201, 168, 76] },
  atRisk:     { start: [192, 57, 43],   end: [231, 76, 60] },
}

function getBandGradient(score) {
  if (score >= 80) return BAND_GRADIENTS.strong
  if (score >= 60) return BAND_GRADIENTS.developing
  if (score >= 50) return BAND_GRADIENTS.fragile
  return BAND_GRADIENTS.atRisk
}
...
export async function generateClientPdf({ scores, report, project, screenerResponses, diagnosticResponses, thresholds }) {
  ...
  function capPillarRefs(text) {
    if (!text) return text
    return text.replace(/\b(p\d+(?:_q\d+)?)\b/gi, match => match.toUpperCase().replace('_Q', '/Q'))
  }
  ...
  function renderLineWithBoldPillars(line, x, yPos, baseFontStyle) {
    ...
    const segments = []
    const pillarPattern = /\b(P\d+(?:\/Q\d+)?)\b/g
    let lastIdx = 0
    let match
  ...
  function drawGradientRect(gx, gy, gw, gh, startColor, endColor, steps = 40) {
    const sliceW = gw / steps
    for (let i = 0; i < steps; i++) {
      ...
      doc.rect(gx + i * sliceW, gy, sliceW + 0.5, gh, 'F')
    }
  }
  ...
  function drawHBarChart(data, opts = {}) {
    ...
    for (const item of data) {
      ...
      // Value bar — gradient fill
      const grad = getBandGradient(item.score)
      const color = getBandColor(item.score)
      const valW = Math.max(barMaxW * (item.score / 100), 1)
      drawGradientRect(barX, y, valW, barHeight, grad.start, grad.end)
      doc.setDrawColor(...color)
    ...
    if (ca) {
      ...
      if (ca.what_good_looks_like?.length) {
        ...
        for (const item of ca.what_good_looks_like) {
          bulletPoint(item, { fontSize: 9, boldLeadIn: false })
        }
      }
