...
import { PILLAR_WEIGHTS, PILLAR_NAMES } from '@/lib/constants'
...
function getBandColor(score) {
  if (score >= activeThresholds.green) return COLORS.green
  if (score >= activeThresholds.yellow) return COLORS.yellow
  if (score >= activeThresholds.orange) return COLORS.orange
  return COLORS.rose
}

function getBandLabel(score) {
  if (score >= activeThresholds.green) return 'Green'
  if (score >= activeThresholds.yellow) return 'Lime Green'
  if (score >= activeThresholds.orange) return 'Dark Orange'
  return 'Red'
}

function getBandName(score) {
  if (score >= activeThresholds.green) return 'Growth Engine Strong'
  if (score >= activeThresholds.yellow) return 'Growth System Constrained'
  if (score >= activeThresholds.orange) return 'Growth System Underpowered'
...
export async function generateClientPdf({ scores, report, project, screenerResponses, thresholds }) {
  ...
  if (raps?.score !== undefined) {
    ...
    if (rapsLabel) {
      ...
      doc.text(labelText, pageW / 2, y + 48, { align: 'center' })
      doc.setFont('helvetica', 'normal')
    }
    y += 56
